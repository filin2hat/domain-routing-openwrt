...
sleep 10
ip route add table vpn default dev vpn-tun
EOF
    fi
}
...
add_tunnel() {
    ...
    if [ "$TUNNEL" == 'singbox' ]; then
        ...
        if grep -q "vpn-tun" /etc/sing-box/config.json; then
...
add_zone() {
    if  [ "$TUNNEL" == 0 ]; then
        ...
        # Delete exists zone
        zone_tun_id=$(uci show firewall | grep -E '@zone.*vpn-tun' | awk -F '[][{}]' '{print $2}' | head -n 1)
        if [ "$zone_tun_id" == 0 ] || [ "$zone_tun_id" == 1 ]; then
            printf "\033[32;1mvpn-tun zone has an identifier of 0 or 1. That's not ok. Fix your firewall. lan and wan zones should have identifiers 0 and 1. \033[0m\n"
            exit 1
        fi
        ...
        if [ "$TUNNEL" == wg ]; then
            ...
            uci set firewall.@zone[-1].device='vpn-tun'
        fi
